...
<div class="space-y-6">
    <div class="card p-4">
        <h2 class="la-heading text-lg font-semibold text-[#f2efe9] mb-3">Owned Materials Management</h2>
        <p class="text-sm text-[#a29e96] mb-4">
        ...
        <div class="space-y-6">
            {#each materials as material (material.id)}
                <div class="border border-[rgba(255,255,255,0.12)] rounded-sm p-4">
                    <div class="flex items-center gap-3 mb-4">
                        <img
                            ...
                            alt={material.name}
                        />
                        <span class="la-heading font-medium text-xl">{material.name}</span>
                    </div>
                    
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                        <!-- Base Owned Section -->
                        <div class="border border-[rgba(255,255,255,0.12)] rounded-sm p-4 lg:col-span-1">
                            <h3 class="la-heading text-md font-medium text-[#f2efe9] mb-3">Base Owned</h3>
                        ...
                        <!-- Bags Section -->
                        <div class="border border-[rgba(255,255,255,0.12)] rounded-sm p-4 lg:col-span-3">
                            <div class="flex justify-between items-center mb-3">
                                <h3 class="la-heading text-md font-medium text-[#f2efe9]">Bags</h3>
                                <button
                                    onclick={() => addBoxType(material.id)}
                                    class="text-[#d0a75a] hover:text-[#e8c987] text-sm flex items-center gap-1"
                                >
                            ...
                            {#if material.boxes?.length}
                                <div class="space-y-3">
                                    {#each material.boxes as box, index}
                                        <div class="flex flex-wrap items-center gap-3 p-3 bg-[rgba(255,255,255,0.02)] rounded-sm">
                                            <div class="flex-1 min-w-[120px]">
                                                <label class="block text-xs font-medium text-[#a29e96] mb-1">Label</label>
                                            <input
                                                type="text"
                                                value={box.label}
                                                oninput={(e) => updateBoxDetail(material.id, index, 'label', (e.target as HTMLInputElement).value)}
                                                class="w-full rounded-sm px-2 py-1 text-sm bg-[rgba(0,0,0,0.35)] border border-[rgba(255,255,255,0.12)] text-[#f2efe9] outline-none focus:border-[#d0a75a]"
                                            />
                                            </div>
                                            
                                            <div class="flex-1 min-w-[80px]">
                                                <label class="block text-xs font-medium text-[#a29e96] mb-1">Size</label>
                                                <NumberInput
                                                    value={box.size}
                                            ...
                                            <div class="flex-1 min-w-[80px]">
                                                <label class="block text-xs font-medium text-[#a29e96] mb-1">Owned</label>
                                                <NumberInput
                                                    value={box.owned}
                                            ...
                                            <button
                                                ...
                                                class="text-[#e05c5c] hover:text-[#ff7a7a] p-1"
                                                title="Remove bag"
                                            >
                            ...
                            {:else}
                                <p class="text-sm text-[#a29e96] italic">No bags defined</p>
                            {/if}
                        </div>
                    </div>
                    
                    <!-- Total Available -->
                    <div class="mt-4 pt-3 border-t border-[rgba(255,255,255,0.12)]">
                        <div class="flex justify-between items-center">
                            <span class="font-medium text-[#a29e96]">Total Owned:</span>
    ...
    <div class="card p-4">
        <h3 class="la-heading text-md font-semibold text-[#f2efe9] mb-2">Quick Actions</h3>
